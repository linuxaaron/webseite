...
import { ArrowUpRight, Shield, Search, BriefcaseBusiness, Database } from "lucide-react";
import { Reveal } from "@/components/reveal";

export const metadata: Metadata = {
  title: "Projekte und Open Source Arbeiten",
  description: "Ausgewählte Open-Source- und Portfolio-Projekte von Joscha Aaron Schmidt zu Cybersecurity, OSINT, Malware-Analyse und Webentwicklung.",
  alternates: { canonical: "/projekte" },
};

const projects = [
  { name: "web-osint", description: "Ein in Go entwickeltes OSINT-Werkzeug für die technische Analyse öffentlich sichtbarer Informationen zu Domains und Websites. Dazu gehören DNS, TLS, Security-Header, erkannte Technologien und strukturierte JSON-Reports.", href: "https://github.com/linuxaaron/web-osint", icon: Search, tags: ["OSINT", "Go", "Web Analysis"] },
  ...
  { name: "portfolio", description: "Mein persönliches Portfolio rund um Webentwicklung, Full Stack Development, KI-Automatisierung, Cybersecurity, OSINT, Linux/Unix, Netzwerktechnik und Open Source.", href: "https://github.com/linuxaaron/portfolio", icon: BriefcaseBusiness, tags: ["Portfolio", "Web Development", "Open Source"] },
];
    ...
    <main className="container py-20 sm:py-28">
      <Reveal>
        ...
        <p className="mt-5 max-w-2xl text-lg leading-8 text-[var(--muted)]">
          Eine Auswahl meiner Open-Source- und Portfolio-Projekte. Im Mittelpunkt stehen praktische Lösungen, nachvollziehbare Technik und eine saubere Umsetzung.
        </p>
      </Reveal>
          ...
          return (
            <Reveal key={project.name} delay={index * 0.06}>
              <a
                ...
                className="group flex h-full flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 transition hover:-translate-y-1 hover:border-[var(--accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
              >
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="rounded-full bg-[var(--surface-2)] px-2.5 py-1 font-mono text-[11px] text-[var(--muted)]">{tag}</span>
                  ))}
                </div>
                <span className="mt-7 inline-flex items-center gap-2 text-sm">Repository <ArrowUpRight size={15} aria-hidden="true" /></span>
              </a>
            </Reveal>
          );
        })}
